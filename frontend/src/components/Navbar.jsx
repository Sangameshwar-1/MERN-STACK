...
const Navbar = () => {
  ...
  const isActive = (path) => location.pathname === path ? 'text-zinc-100' : 'text-zinc-400 hover:text-zinc-200';
  ...
  return (
    <nav className="sticky top-0 z-50 w-full border-b border-zinc-800 bg-[#09090b]/80 backdrop-blur-xl">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          <div className="flex items-center">
            <Link to={user ? (user.role === 'admin' ? '/admin' : user.role === 'organizer' ? '/organizer' : '/participant') : '/'} className="flex items-center gap-2">
              ...
              <span className="text-xl font-bold tracking-tight text-zinc-100 hidden sm:block">Felicity</span>
            </Link>
          </div>
        ...
        {/* Mobile Toggle */}
        <button 
          className="md:hidden p-2 -mr-2 text-zinc-400 hover:text-zinc-100" 
      ...
      {/* Mobile Navigation */}
      {menuOpen && (
        <div className="md:hidden border-t border-zinc-800 bg-[#09090b] px-4 py-4 space-y-4">
          {user ? (
